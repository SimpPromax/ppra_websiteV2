// src/pages/ResourceCentre/ActiveSTDs.jsx
...
const ActiveSTDs = () => {
  ...
  const { activeDocuments, loading, error } = state;
  ...
  const extractYear = (doc) => {
    // For active documents, year is in the tab name (2022 or 2021)
    // But we can also extract from title
    const yearMatch = doc.title.match(/\b(20\d{2})\b/);
    if (yearMatch) {
      return parseInt(yearMatch[1]);
    }
    // Fallback to tab name if available
    if (doc.tab && doc.tab !== 'old') {
      return parseInt(doc.tab);
    }
    return 0;
  };

  // ===== GET ALL ACTIVE DOCUMENTS WITH YEAR =====
  const getAllActiveDocs = useMemo(() => {
    const docs = [];
    // Get 2022 documents
    (state.documents?.['2022'] || []).forEach(doc => {
      docs.push({ ...doc, year: 2022 });
    });
    // Get 2021 documents
    (state.documents?.['2021'] || []).forEach(doc => {
      docs.push({ ...doc, year: 2021 });
    });
    return docs;
  }, [state.documents]);
  ...
  const filteredDocuments = useMemo(() => {
    ...
    if (selectedYear !== 'All') {
      ...
      filtered = filtered.filter(doc => doc.year === yearNum);
    }
    ...
    return filtered;
  }, [getAllActiveDocs, searchTerm, selectedYear]);
  ...
  useEffect(() => {
    ...
    if (heroRef.current) {
      gsap.fromTo(heroRef.current.querySelector('.active-std-hero_heading'),
        { y: 50, opacity: 0 },
        {
          y: 0,
          opacity: 1,
    ...
    return (
      <div className="page-wrapper bg-white min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading Active Standard Tender Documents...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-wrapper bg-white">
      <Helmet>
        <title>Active Standard Tender Documents | PPRA Kenya</title>
        <meta name="description" content="Access active Standard Tender Documents - 2022 and 2021 versions for public procurement in Kenya." />
        <meta name="keywords" content="standard tender documents, active STDs, PPRA, procurement, Kenya" />
        <meta property="og:title" content="Active Standard Tender Documents - PPRA Kenya" />
        <meta property="og:description" content="Active standard tender documents for public procurement in Kenya." />
        <meta property="og:type" content="website" />
        <meta property="og:image" content={logoImage} />
        <link rel="canonical" href="https://ppra.go.ke/active-standard-tender-documents" />
      </Helmet>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .active-doc-item .doc-info {
            width: 100%;
          }
          .active-doc-item .doc-download {
      ...
      <main className="main-wrapper">
        ...
        {/* ===== HERO SECTION ===== */}
        <section className="section-active-std-hero relative pt-8">
          ...
          <div className="padding-global z-index-1 relative px-4 md:px-6 lg:px-12">
            <div className="container-large max-w-7xl mx-auto">
              <div ref={heroRef} className="active-std-hero_component relative h-[45vh] md:h-[50vh] lg:h-[55vh] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 w-full h-full">
                  <img src={corporateSky} alt="Active Standard Tender Documents" className="w-full h-full object-cover" loading="eager" />
                </div>
                ...
                <div className="active-std-hero_heading max-w-4xl mx-auto text-center z-10 px-4">
                  ...
                  <h1 className="heading-style-h1 text-white text-3xl md:text-5xl lg:text-6xl font-bold animate-fadeInUp">
                    Active Standard Tender Documents
                  </h1>
                  <p className="text-white text-sm md:text-xl mt-2 md:mt-4 opacity-90 max-w-2xl mx-auto animate-fadeInUp" style={{ animationDelay: '0.2s' }}>
                    2022 &amp; 2021 versions for public procurement
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* ===== CONTENT SECTION ===== */}
        <section className="section-active-std-content relative bg-white">
          <div className="line-wrapper is-invert absolute inset-0 pointer-events-none flex">
            <div className="vertical-line w-1/5 border-r border-gray-200"></div>
            <div className="vertical-line w-1/5 border-none"></div>
            <div className="vertical-line w-1/5 border-none"></div>
          ...
          <div className="z-index-1 relative">
            <div className="padding-global padding-section-large px-4 md:px-6 lg:px-12 py-10 md:py-24">
              <div className="container-large max-w-5xl mx-auto">
                <div ref={containerRef} className="active-std-component">
                  ...
                  <div className="text-center mb-8 md:mb-12">
                    ...
                    <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto mt-4">
                      Browse through 2022 and 2021 standard tender documents
                    </p>
                  </div>
                    ...
                    <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'space-y-3'}>
                      {filteredDocuments.map((doc, index) => (
                        viewMode === 'grid' ? (
                            ...
                            <div className="flex items-start gap-3 flex-1">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-gray-900 leading-snug">
                                  {doc.docNumber && <span className="text-primary-purple font-bold">{doc.docNumber}</span>}
                          ...
                          <div 
                            key={`${doc.id}-${index}`}
                            className="active-doc-item bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all p-4 md:p-5"
                          >
