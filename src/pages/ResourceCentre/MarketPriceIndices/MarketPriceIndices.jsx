// src/pages/ResourceCentre/MarketPriceIndices.jsx
...
// ============================================================
// SERVICE: Fetch Market Price Indices from WordPress
// ============================================================
const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000';
const PROXY_ENDPOINT = `${BACKEND_URL}/api/proxy/market-price-index`;
...
class MarketPriceService {
  ...
  async fetchIndices(options = {}) {
    const { forceRefresh = false } = options;

    // Check cache first
    if (!forceRefresh && this.isCacheValid()) {
      console.log('✅ [Cache] Using cached market price indices data');
    ...
    try {
      ...
      this.loading = false;
      this.notifyListeners();
      console.log(`✅ Successfully fetched ${result.length} market price indices`);
      this.debugStats(); // Auto-log stats
      return result;
    } catch (error) {
      console.error('❌ Error fetching market price indices:', error);
      this.error = error.message || 'Unable to fetch market price indices.';
      this.loading = false;
      this.notifyListeners();
      throw error;
    } finally {
  ...
  async _fetchFromProxy() {
    console.log('🔍 Fetching market price indices from proxy:', PROXY_ENDPOINT);
    ...
    try {
      ...
      const files = this.parseIndicesFromHTML(html);
  ...
  parseIndicesFromHTML(html) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(html, 'text/html');
    const files = [];

    // Find all WPDM download items
    ...
    // Sort by date (newest first) - extract year/month from title
    files.sort((a, b) => {
  ...
  extractFileData(item) {
    try {
      ...
      // Extract title text and size
      const titleText = title.textContent?.trim() || '';
      const sizeText = sizeLabel?.textContent?.trim() || '';

      return {
        id: `mpi-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`,
        title: titleText,
        size: sizeText,
        ...
        month: this.extractMonth(titleText),
      };
    } catch (error) {
  ...
  extractMonth(title) {
    ...
    for (const month of months) {
      if (title.toUpperCase().includes(month)) {
        return month;
      }
    }
    return null;
  ...
  extractDateFromTitle(title) {
    ...
    const month = this.extractMonth(title);
    if (year) {
      const monthIndex = ['JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY', 'JUNE', 
                          'JULY', 'AUGUST', 'SEPTEMBER', 'OCTOBER', 'NOVEMBER', 'DECEMBER']
                          .indexOf(month || 'JANUARY');
  ...
  debugStats() {
    ...
    console.log('📊 MARKET PRICE INDICES - DATA LOADED');
    console.log('='.repeat(60));
...
// REACT COMPONENT
// ============================================================
const MarketPriceIndices = () => {
  const heroRef = useRef(null);
  const containerRef = useRef(null);

  // Service state
  const [state, setState] = useState(() => marketPriceService.getState());
  ...
  useEffect(() => {
    const unsubscribe = marketPriceService.subscribe((newState) => {
      setState(newState);
    });
    marketPriceService.fetchIndices();
    return unsubscribe;
  }, []);
  ...
  useEffect(() => {
    ...
    if (heroRef.current) {
      gsap.fromTo(heroRef.current.querySelector('.mpi-hero_heading'),
        { y: 50, opacity: 0 },
        {
          y: 0,
          opacity: 1,
    ...
    return (
      <div className="page-wrapper bg-white min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading Market Price Indices...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="page-wrapper bg-white min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-2">Unable to Load Indices</h2>
          <p className="text-gray-600 mb-4">{error}</p>
          <button
            onClick={() => marketPriceService.fetchIndices({ forceRefresh: true })}
  ...
  return (
    <div className="page-wrapper bg-white">
      <Helmet>
        <title>Market Price Indices | PPRA Kenya</title>
        <meta name="description" content="Access PPRA Market Price Indices - reference price guides for public procurement in Kenya." />
        <meta name="keywords" content="market price indices, PPRA, procurement prices, Kenya, public procurement, price guide" />
        <meta property="og:title" content="Market Price Indices - PPRA Kenya" />
        <meta property="og:description" content="Reference price guides for public procurement in Kenya." />
        <meta property="og:type" content="website" />
        <meta property="og:image" content={logoImage} />
        <link rel="canonical" href="https://ppra.go.ke/market-price-index" />
      </Helmet>
      ...
      <main className="main-wrapper">
        ...
        {/* ===== HERO SECTION ===== */}
        <section className="section-mpi-hero relative pt-8">
          ...
          <div className="padding-global z-index-1 relative px-4 md:px-6 lg:px-12">
            <div className="container-large max-w-7xl mx-auto">
              <div ref={heroRef} className="mpi-hero_component relative h-[45vh] md:h-[50vh] lg:h-[55vh] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 w-full h-full">
                  <img src={corporateSky} alt="PPRA Market Price Indices" className="w-full h-full object-cover" loading="eager" />
                </div>
                ...
                <div className="mpi-hero_heading max-w-4xl mx-auto text-center z-10 px-4">
        ...
        {/* ===== CONTENT SECTION ===== */}
        <section className="section-mpi-content relative bg-white">
          <div className="line-wrapper is-invert absolute inset-0 pointer-events-none flex">
            <div className="vertical-line w-1/5 border-r border-gray-200"></div>
            <div className="vertical-line w-1/5 border-none"></div>
            <div className="vertical-line w-1/5 border-none"></div>
          ...
          <div className="z-index-1 relative">
            <div className="padding-global padding-section-large px-4 md:px-6 lg:px-12 py-10 md:py-24">
              <div className="container-large max-w-4xl mx-auto">
                <div ref={containerRef} className="mpi-component">
...
export default MarketPriceIndices;