// src/services/strategicPlanService.js
/**
 * Service for fetching Strategic Plan documents from PPRA website
 * Uses local backend proxy for CORS-free access
 */

const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000';
const PROXY_ENDPOINT = `${BACKEND_URL}/api/proxy/strategic-plan`;
...
class StrategicPlanService {
  ...
  async fetchDocuments(options = {}) {
    ...
    if (!forceRefresh && this.isCacheValid()) {
      console.log('✅ [Cache] Using cached strategic plan data');
    ...
    try {
      ...
      this.loading = false;
      this.notifyListeners();
      console.log(`✅ Successfully fetched ${result.length} strategic plan documents`);
      this.debugStats();
      return result;
    } catch (error) {
      console.error('❌ Error fetching strategic plan documents:', error);
      this.error = error.message || 'Unable to fetch strategic plan documents.';
      this.loading = false;
      this.notifyListeners();
      throw error;
    } finally {
  ...
  async _fetchFromProxy() {
    console.log('🔍 Fetching strategic plan from proxy:', PROXY_ENDPOINT);
    ...
    try {
      ...
      const files = this.parseDocumentsFromHTML(html);
  ...
  extractFileData(item) {
    try {
      ...
      return {
        id: `strategic-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`,
        title: titleText,
        size: sizeText,
  ...
  debugStats() {
    ...
    console.log('📊 STRATEGIC PLAN - DATA LOADED');
    console.log('='.repeat(60));
    console.log(`📊 Total Documents: ${this.files.length}`);
...
const strategicPlanService = new StrategicPlanService();
